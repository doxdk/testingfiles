...
$tempScriptPath = "$env:TEMP\hiddenScript.ps1"
$scriptContent | Out-File -FilePath $tempScriptPath -Encoding UTF8

# Ejecutar el script en segundo plano sin mostrar la ventana
Start-Process powershell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$tempScriptPath`"" -WindowStyle Hidden
